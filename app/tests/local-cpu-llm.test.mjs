...
test('CPU-Fallback bleibt Vulkan-unabhängig und offline cachebar', () => {
  ...
  assert.match(source, /globalThis\.crossOriginIsolated/)
  assert.match(source, /n_threads:\s*cpuThreads/)
  assert.match(source, /n_batch:\s*cpuThreads\s*>\s*1\s*\?\s*256\s*:\s*128/)
...
test('Produktchat begrenzt CPU-Arbeit ohne die Experimentdefaults zu ändern', () => {
  ...
  assert.match(conversation, /startsWith\('native:'\)/)
