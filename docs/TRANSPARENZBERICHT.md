...
| Live sprechen statt nur diktieren | Der alte Mikrofonknopf nahm nur eine einzelne Frage auf. Ein echtes gleichzeitiges Hören und Vorlesen hätte auf Handys Rückkopplungen erzeugt; außerdem arbeitet die Browser-Spracherkennung nicht garantiert lokal. | ChatGPT implementierte den Ablauf „zuhören → transkribieren → Graph-RAG/Modell → vorlesen → erneut zuhören“, Zwischen-Transkripte, Fehlerklassen, Lifecycle-Abbruch und einen responsiven Voice-Dialog. Bei der Prüfung fielen ein Startkonflikt nach sehr schnellem Pause/Fortsetzen und eine zunächst zu späte Datenschutzinformation auf; beides wurde korrigiert und erneut getestet. | Ich akzeptiere bewusst einen turn-basierten statt eines technisch übertrieben dargestellten Realtime-Audiomodus. Im Offline-Modus bleibt das Mikrofon gesperrt; online öffnet der erste Klick nur die Information und erst der zweite startet den Browserdienst. Beim Wechsel der Ansicht, Verbergen der Seite oder Beenden stoppt die App Mikrofon, Vorlesen und laufende Antwort. |
...
| Eigenes lokales Wissen | Der erste PDF-Import schnitt längere Dokumente still ab, verlor Seitenbelege und hinterließ bei Reimporten alte Abschnittsknoten. | Umsetzung von lokalem PDF-Parsing, SHA-256-Fingerprint, Provenienz, atomarem Source-Scope-Reimport, Delta-Bericht und animierter Graphansicht. | Ich verlangte sichtbare Belege und konservative Kanten: Dokumentstruktur oder eindeutige Namensnennung statt frei erfundener Beziehung; OCR und komplexes Spaltenlayout bleiben offen. |
