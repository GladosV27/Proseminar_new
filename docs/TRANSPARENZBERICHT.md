...
| Live-Sprachdialog | ChatGPT | Entwurf und Implementierung einer Zustandsmaschine für Spracherkennung, Modellantwort, Vorlesen, Pause, Unterbrechung und mobilen Vollbilddialog; später Integration einer optional geladenen, lokal inferierenden Piper-Stimme mit sichtbarer Datenschutzgrenze | Ich verlangte ein Gesprächserlebnis ähnlich einem modernen Voice-Chat, entschied mich nach Abwägung gegen vorgetäuschtes Voll-Duplex und für den stabileren turn-basierten Ablauf. Nach dem unnatürlichen Klangeindruck der Samsung-Stimme forderte ich eine freie Alternative. Ich prüfte Build und automatisierte Tests; Installation, Klang und Abbruchverhalten auf meinem S23+ sind noch ⬜. |
...
| Themenwechsel im Gespräch | Die Heuristik deutete jedes „es“ als Rückverweis. Dadurch wurde „Wie viele Etagen hat der Mathetower … Falls du es nicht weißt …“ mit der vorherigen Einstein-Frage verkettet; zusätzlich sah das kleine Modell den alten Verlauf und wiederholte Einstein. | ChatGPT entfernte das generische „es“ aus der Anschlussheuristik, trennt unabhängige Fragen vollständig vom alten Modellverlauf und erkennt den ausdrücklichen Wikipedia-Fallback als bewusste Rechercheanweisung. Der originale Mathetower-Satz und eine echte Pronomen-Anschlussfrage wurden als Gegentests ergänzt. | Ich werte das nicht als bloße Modellhalluzination, sondern als Fehler meiner vorgelagerten Kontextlogik. Der Fix verändert nur den Produktchat; das Experiment arbeitet weiterhin mit unabhängigen Einzeltrials. |
| Umschaltbares Retrieval und Antwortdauer | Der Produktchat nutzte immer Graph-RAG; beim CPU-Modell wurden bis zu 14 Knoten, sechs längere Verlaufsnachrichten und 180 Ausgabetokens auf nur einem WASM-Thread langsam verarbeitet. Ein erster Optimierungsversuch mit 112 Tokens kappte jedoch eine Einstein-Antwort sichtbar mitten im Satz. | ChatGPT ergänzte Automatik, Vektor-, Graph- und Hybrid-RAG samt sichtbarer Kontext-/Zeitangabe. Für CPU-Chatantworten begrenzte es den Teilgraphen auf acht Knoten und die Historie auf vier kompakte Nachrichten. Nach dem Abbruchfehler erhöhte es das Chatbudget auf 144 Tokens, verlangte höchstens drei kurze Sätze und ergänzte eine getestete Abschlusslogik, die ein verbleibendes Fragment bis zum letzten vollständigen Satz entfernt. | Ich behandle diese Einstellungen ausdrücklich als UX-Optimierung des Chats und dokumentiere die misslungene Zwischeniteration. Experimentläufe übergeben keine Chat-Limits und behalten ihre eingefrorenen Retrieval- und Generierungsdefaults; dadurch werden alte und neue Messläufe nicht vermischt. |
| Live sprechen statt nur diktieren | Der alte Mikrofonknopf nahm nur eine einzelne Frage auf; die erste Browserstimme klang außerdem monoton und las Markdown-Zeichen mit vor. Ein echtes gleichzeitiges Hören und Vorlesen hätte auf Handys Rückkopplungen erzeugt, und die Browser-Spracherkennung arbeitet nicht garantiert lokal. | ChatGPT implementierte den Ablauf „zuhören → transkribieren → Graph-RAG/Modell → vorlesen → erneut zuhören“, Zwischen-Transkripte, Fehlerklassen und Lifecycle-Abbruch. Nach mehreren Prosodie-Verbesserungen blieb die S23+-Systemstimme unbefriedigend; deshalb ergänzte ChatGPT die freie Piper-Stimme »Thorsten« als bewussten Download mit lokaler ONNX-/WASM-Inferenz, Fortschritt, Abbruch und Browserstimmen-Fallback. | Ich akzeptiere bewusst einen turn-basierten statt eines technisch übertrieben dargestellten Realtime-Audiomodus. Die Ausgabe kann nach dem Download lokal entstehen; die Mikrofonerkennung bleibt eine getrennte Browserfunktion und ist deshalb im Offline-Modus weiter gesperrt. Die Stimme ist ein Demo-Feature und verändert keine Versuchsbedingung. |
...
| Interaktiver Graph und Live-Arena | Der ursprüngliche Canvas hatte keine Kamera; Mauskoordinaten entsprachen direkt Graphkoordinaten. Freies Navigieren, Zoomen und Touch-Gesten fehlten. | ChatGPT implementierte View-Transformation, cursorzentrierten Zoom, Hintergrund-Pan, Node-Drag, Pinch-Zoom, Fit/Reset, Ebenenfilter, animierte Importdifferenz, Evidenzansicht und eine verblindete A/B-Arena. | Ich verlangte, dass die technische Demonstration nicht nur hübsch, sondern methodisch lesbar bleibt. Satzzuordnungen sind als lexikalische Hilfsansicht gekennzeichnet; eine Kanten-Gegenprobe verändert sichtbar nur die gewählte Kante. |
| Speicherung, Gerätetest und Tests | `localStorage` allein war für wachsende PDF-/Graphdaten fragil; außerdem meldeten manche Android-Geräte WebGPU, scheiterten aber erst beim Compute-Pipelinebau. | Umsetzung eines IndexedDB-Spiegels mit Zeitstempelabgleich, eines echten WebGPU-Mini-Compute-Tests und automatisierter Node-Tests für Aktionsparser und Graph-Traversal. | Ich behielt den synchronen lokalen Rückfall bei, ließ Builds nach jeder Integrationsstufe laufen und nahm Tests als verpflichtenden Schritt vor das GitHub-Pages-Deployment auf. |
| Gemeinsamer Seminargraph | Ein QR-Quiz ließ Handys teilnehmen, aber die Gruppe konnte den Wissensgraphen nicht gemeinsam erweitern. | ChatGPT ergänzte Realtime-Themenvorschläge und eine Host-Freigabe vor dem Wikipedia-Import. | Ich entschied mich gegen einen ungeprüften Publikumsimport: Teilnehmende senden nur einen Themenbegriff; erst meine Freigabe löst den öffentlichen Netzabruf auf dem Präsentationsgerät aus. |
...
Der Umfang entspricht einem Proseminar: ein manuell kuratierter Wissensgraph, **eine deutsche Sprache**, eine Domäne und n = 40 Fragen. Die verwendeten Sprachmodelle können technisch auch Englisch verarbeiten; daraus folgt aber keine englische Validierung. Fragen, Goldantworten, Scoring-Begriffe und Prompts des Experiments bleiben deutsch. Die Stichprobe erlaubt keine allgemeingültige Aussage über Graph-RAG oder Wikipedia insgesamt. Die manuelle Kuratierung verbessert die Kontrolle, begrenzt aber die Übertragbarkeit und kann meine Erwartungen in den Korpus eingebracht haben. Browser- und Smartphone-Latenzen hängen außerdem von Gerät, Hintergrundlast und Temperatur ab. Ich dokumentiere deshalb Gerätebedingungen, Wiederholungen und p50/p95 und berichte auch Null- oder Negativbefunde unverändert. Der Live-Sprachdialog ist eine Bedienfunktion außerhalb des Experiments: Seine Piper-Ausgabe kann lokal laufen, die Web-Speech-Eingabe ist aber nicht garantiert offline und der Ablauf ist kein echtes gleichzeitiges Realtime-Audiostreaming. Weitere Limitationen stehen in der Ausarbeitung, Abschnitt 7.
