...
Im Offline-Modus prueft die App vor dem Laden eines WebLLM-Modells, ob alle Modellartefakte im Browser-Cache vorhanden sind. Fehlt etwas, wird kein Downloadversuch gestartet. Beim Embedding-Modell ist der Fernzugriff ebenfalls deaktiviert; eine vorhandene Browser-Kopie kann geladen werden, eine fehlende fuehrt zu einer lokalen Fehlermeldung.
...
2. Die angezeigte `localhost`-Adresse im Browser oeffnen. Fuer die PWA- und Cache-Funktionen ist die Produktionsversion wichtig; `npm.cmd run dev` ist fuer Entwicklung gedacht.
